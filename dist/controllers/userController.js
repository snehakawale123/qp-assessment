...
export const bookItems = async (req, res) => {
    ...
    try {
        const insertPromises = items.map(async (item) => {
            ...
            connection.query("SELECT itemId FROM grocery_items WHERE itemId = ?", [itemId], (error, results) => {
                if (error) {
                    res.status(500).json({ error: "Failed to retrieve grocery items" });
                    return;
                }
                if (results.length > 0) {
                    ...
                    connection.query("INSERT INTO booked_items (userId, itemId, quantity) VALUES (?, ?, ?)", [userId, itemId, quantity], (error, results) => {
                        if (error) {
                            res
                                .status(500)
                        ...
                        if (results.affectedRows > 0) {
                            ...
                            connection.query("UPDATE grocery_items SET inventory = inventory - ? WHERE itemId = ?", [quantity, itemId], (error, results) => {
                                if (error) {
                                    res
                                        .status(500)
                                        .json({ error: "Failed to retrieve grocery items" });
